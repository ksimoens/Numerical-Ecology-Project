...
sel.dbMEM <- forward.sel(freq.atl, dbMEM, adjR2thresh=R2adj.dbMEM, alpha=0.06, nperm=9999)
print(sel.dbMEM)
...
rda.part <- varpart(freq.atl, linear, dbMEM.sel, AEM.sel)
...
plotRDA(df.plot.s1,Evalues,species.scores,variables,scale.fac,"RDA_freq_neut_s1_country.png",3,1)
...
p %>% ggsave("resid_PCA_freq_neut.png",.,device='png',width=15,height=10,units='cm')

rdacca <- rdacca.hp(freq.atl,list(as.data.frame(linear), as.data.frame(dbMEM.sel), as.data.frame(AEM.sel)), method='RDA')
...
p %>% ggsave("rdacca_sep_freq_neut.png",.,device='png',width=15,height=7.5,units='cm')
